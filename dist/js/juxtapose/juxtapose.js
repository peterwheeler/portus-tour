...
(function (document, window) {
  ...
  function Graphic(properties, slider) {
    var self = this;
    this.object = {};
    this.object.image = new Image();

    this.loaded = false;

    this.object.image.src = properties.src;
    this.object.label = properties.label || false;
    this.object.credit = properties.credit || false;
    this.object.id = properties.id || false;

    this.object.image.onload = function() {
      self.loaded = true;
      slider._onLoaded(self.object);
    };
  }
  ...
  function JXSlider(selector, images, options) {
    ...
    if (this.images.every(this.allLoaded) === true && this.imgBefore && this.imgAfter) {
      setTimeout(function(){self._allLoaded();}, 3000);
  ...
  JXSlider.prototype = {
    ...
    checkImages: function() {
      if (getImageDimensions(this.imgBefore.object.image).aspect() ==
        getImageDimensions(this.imgAfter.object.image).aspect()) {
        return true;
      } else {
        return false;
      }
    ...
    calculateDims: function(width, height){
      var ratio = getImageDimensions(this.imgBefore.object.image).aspect();
      if (width) {
        height = width / ratio;
      } else if (height) {
        width = height * ratio;
    ...
    _allLoaded: function(){
      ...
      this.container.className = 'juxtapose-container';
console.log(getNaturalDimensions(this.imgBefore.object.image));
      this.wrapper = document.querySelector(this.selector);
      addClass(this.wrapper, 'juxtapose');
      this.wrapper.appendChild(this.container);
      this.wrapper.style.width = getNaturalDimensions(this.imgBefore.object.image).width;
      this.setWrapperDimensions();
